...
describe UserAgent do
  ...
  describe '#browser' do
    examples = {
      ...
      "ChromeFrame"       => [
                              "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.0; Trident/4.0; FunWebProducts; SLCC1; .NET CLR 2.0.50727; Media Center PC 5.0; .NET CLR 3.0.04506; InfoPath.1) chromeframe",
                              "Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.0; WOW64; SLCC1; .NET CLR 2.0.50727; Media Center PC 5.0; .NET CLR 3.5.21022; .NET CLR 3.5.30729; .NET CLR 3.0.30618) chromeframe"
                              ],
      ...
      "Safari 2.0.1"      => "Mozilla/5.0 (Macintosh; U; PPC Mac OS X; fr) AppleWebKit/412.7 (KHTML, like Gecko) Safari/412.5",
      "Safari Mobile"     => ["Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.0.3) Gecko/2008092816 Mobile Safari 1.1.3", "Mozilla/5.0 (iPhone; U; CPU iPhone OS 2_1 like Mac OS X; fr-fr) AppleWebKit/525.18.1 (KHTML, like Gecko) Mobile/5F136"],
    ...
    examples.each do |expected, agents|
      [agents].flatten.each do |agent|
        it "should identify #{expected} from '#{agent}'" do
          UserAgent.new(agent).browser.should eql(expected)
        end
      end
    end
  end
  ...
  describe '#os' do
    ...
    examples = {
      "Linux"           => ["Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.0.2) Gecko/2008092313 Ubuntu/8.04 (hardy) Firefox/3.1.6", "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.0.3) Gecko/2008092816 Mobile Safari 1.1.3"],
      ...
      "Mac OS X 10.5"   => "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.5; en-US; rv:1.9.0.10) Gecko/2009042315 Firefox/3.0.10",
    ...
    examples.each do |expected, agents|
      [agents].flatten.each do |agent|
        it "should identify #{expected} from '#{agent}'" do
          UserAgent.new(agent).os.should eql(expected)
        end
      end
    end
  end
